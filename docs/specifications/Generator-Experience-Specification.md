...
## Streaming Generation **(new — forward specification)**

Today generation is a single request/response with an indeterminate loading skeleton. This spec upgrades that to a streamed experience, since a multi-artifact generation (SQL + Drizzle + JSON + Docs + ERD) is exactly the kind of longer-running AI task where perceived latency benefits most from incremental feedback.

- **Behavior:** as the backend produces each artifact, it appears in `OutputTabs` immediately, with its tab marked complete (a small `--accent-emerald` dot on the tab) while remaining artifacts still show their loading skeleton in place. The user can read/copy an already-completed artifact (e.g., SQL) while the ERD is still rendering — the UI never blocks fully-ready content behind slower-to-produce content.
- **Order:** artifacts stream in a fixed, predictable order (SQL → Drizzle → JSON → Documentation → ERD) so users learn what to expect run over run — unpredictable ordering would undermine the "AI should never surprise users negatively" principle.
- **Fallback:** if the backend cannot stream (a batch response), the UI degrades gracefully to today's single-skeleton behavior — streaming is a progressive enhancement to the experience, not a hard requirement for correctness.
