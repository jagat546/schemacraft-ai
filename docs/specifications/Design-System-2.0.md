...
| `--accent-sky` | `#0284c7` *(new)* | **Info.** Neutral informational callouts, tips, keyboard-shortcut hints, non-actionable status badges. Introduced because no existing token covers this purpose; chosen to match the saturation/weight of the other three accents so it reads as part of the same family, not a foreign addition. | Implemented (S4-001). |

**Corrected value, S4-001A:** this token was originally specified as `#0ea5e9`. Implementation-time WCAG verification (S4-001) found that value measures only 2.66:1 against `--surface-0` and 2.77:1 against `--surface-2` in light mode — below even this document's own 3:1 UI-component-boundary minimum (§11), let alone 4.5:1. `#0284c7` (a darker shade in the same hue) clears 3:1 against `--surface-0`/`--surface-2` in both light and dark mode (3.92–4.86:1), matching the contrast level `--accent-violet` and `--accent-rose` already clear against the same surfaces. No other property of the token changed — same semantic role, same family fit, same usage rules.
