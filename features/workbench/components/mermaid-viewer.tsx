...
import { MermaidCanvas } from "@/features/workbench/components/mermaid-canvas"
...
export function MermaidViewer({ content }: { content: string }) {
  ...
  return (
    <OutputViewerFrame label={config.label} content={content} variant="mermaid">
      {state.status === "loading" && (
        <p className="p-4 text-sm text-muted-foreground">Rendering diagram…</p>
      )}
      {state.status === "success" && (
        // mermaid.render() only returns a serialized SVG string - there is
        // no React-element-based API - so this is the one place in the app
        // where injecting markup is unavoidable. securityLevel: "strict"
        // (set above) is Mermaid's own built-in mitigation for this, since
        // the diagram source is model-generated, not hand-authored. See
        // the architectural notes for why DOMPurify is intentionally not
        // layered on top at this stage. MermaidCanvas gives the injected
        // SVG an accessible name/role it otherwise has none of, and wraps
        // it in pan/zoom (Engineering Spec §9, M5).
        <MermaidCanvas svg={state.svg} label="Entity-relationship diagram" />
      )}
      {state.status === "error" && (
