...
**Last verified:** 2026-07-28, after a live private-beta release-readiness pass (real browser + real dev/staging database verification, not simulated) — see `docs/planning/Private-Beta-Release-Checklist.md`. Branch `release/private-beta-v0.8.0-beta.1`.
...
Updated after the private-beta release-readiness pass (2026-07-28) to reflect what was resolved. Remaining items are inputs for future planning, not re-assessed or expanded here:

- ~~`check_authenticated_rate_limit()`/`delete_own_account()` unapplied to any live database~~ — **Resolved for dev/staging, 2026-07-28.** Applying `supabase/rls.sql` for the first time against a real database surfaced a real gap: `user_preferences` had no table-creation migration anywhere (not Drizzle, not inline SQL), so the whole script failed before ever reaching these two functions. Generated the missing migration (`drizzle/migrations/0002_nappy_raza.sql`, covering `user_preferences` and `generation_rate_limit_events` — the latter's own inline `CREATE TABLE` in `rls.sql` was removed in favor of this, matching how `sandbox_generations` was already done); both functions are now confirmed present and callable in the dev/staging database (`pg_proc` lookup, plus a live end-to-end generation and a live burst-limit exhaustion test). **Production still needs the same migration + `npm run db:apply-sql` run against it, with explicit sign-off.**
...
- **One prepared-but-not-applied database change remains**: a Postgres trigger (S4-009) — still deliberately unapplied, pending human sign-off. `user_preferences`, `generation_rate_limit_events`, `check_authenticated_rate_limit()`, and `delete_own_account()` are now applied and confirmed live in dev/staging (see above); production is the remaining gap.
- ~~Delete-account decision resolved (AD-005), implementation still pending~~ — **Implemented, Sprint 6 (S6-003), smoke-tested live in dev/staging, 2026-07-28.** `delete_own_account()`, `deleteAccountAction`, and the Account Settings confirmation dialog all exist exactly per AD-005's recommendation, and the function is now confirmed present and callable in a real (non-production) database — AD-005's own disclosed verification gap is closed for dev/staging; production still needs the same migration.
...
- ~~Icon-only buttons don't meet the 44×44px touch-target minimum (TD-020)~~ — **Resolved, Sprint 6 (S6-006).** Every icon-only `Button` size variant now carries a 44px hit-slop pseudo-element; tightly-packed clusters (Output actions, Workbench generation nav, split-pane collapse chevrons, Mermaid zoom controls) had their gaps widened one step to reduce hit-slop overlap between neighbors. No live pixel-precision touch test on a real device has been performed; a live browser pass on 2026-07-28 confirmed the hit-slop renders and doesn't visibly break layout, but that's not the same as a real-touchscreen confirmation.
...
- ~~Contrast (Design System 2.0 §11) was not verified with a real rendering pass~~ — **Partially resolved, 2026-07-28.** A live browser pass (computed-style contrast ratios against WCAG AA, both themes, real authenticated session) found and fixed a real, severe failure: `outline`/`ghost` buttons rendered light-mode text in genuine dark mode (1.12:1 measured ratio) — both variants now declare an explicit rest-state `text-foreground`. Remaining, disclosed as lower-severity: `--text-muted` text sits around 4.0–4.1:1 in both themes (just under the 4.5:1 AA threshold for normal text) — a consistent, non-regressed design-system choice, not fixed in this pass (see `docs/planning/Private-Beta-Release-Checklist.md`).
- **Live browser/interactive verification of the Workbench chrome, mobile responsiveness, and core authenticated flows was performed 2026-07-28** — Dashboard, Generator, Workbench, History, Edit & Regenerate, and rate-limit rejection were all confirmed live against a real dev/staging database. Mobile viewport testing specifically was blocked by a browser-automation tool limitation (window resize didn't change the reported viewport); substituted with a static code review (no fixed large pixel widths or oversized min-width utilities found) — still not a substitute for a real device pass.
...
**Known residual gaps from Sprint 7, updated after the 2026-07-28 release-readiness pass:** the SQL-application gap is now closed for dev/staging (see §9) — production is the remaining step. A live browser pass that same day (real dev/staging database, real authenticated session) found and fixed two further real bugs beyond Sprint 7's own scope: a severe dark-mode contrast failure on `outline`/`ghost` buttons, and Account Settings incorrectly showing "Project Settings" in its own top bar. See §9 and `docs/releases/v0.8.0-beta.1-Release-Candidate.md` for the full account.
